...
import {
  ...
  flattenVisibleTree, keyToString, multipleNodeKeysSeparator,
  type NodeItemIdentifier
...
import {fetchAllContacts} from "@/utils/ContactApiUtils.ts";
import {createContactPath, settingsPath} from "@/router/paths.ts";

const contacts = reactive<Contact[]>(findAllContacts())
...
watch(
  ...
  currentExpandedNodes => {
    localStorage.setItem(
      "expandedNodes",
      Array.from(currentExpandedNodes).reduce((previousValue, currentValue) => {
...
watch(
  ...
  root => {
    ...
    const previousExpandedNodes = localStorage.getItem("expandedNodes")
    if (previousExpandedNodes === null) {
      ...
      previousExpandedNodes.split(multipleNodeKeysSeparator).forEach(
        previousExpandedNode => expandedNodes.add(previousExpandedNode)
      )
    }
  },
  { immediate: true }
...
function getArrowLeftToDetails(contactId: bigint): boolean {
  return contactIdToArrowLeftToDetails.get(contactId) ?? false
}
...
function deleteContact(id: bigint) {
  store.contactApiService.deleteContact(id).then(response => {
    if (response.status === 200) {
      ...
      const oldValue = focusedItem.value
      moveFocus(-1)
      if (focusedItem.value === oldValue) {
        moveFocus(1)
      }
    }
  })
      ...
      >
        ...
        >
          <ContactDetails
            ...
            :resetArrowLeft="(contactId: bigint) => contactIdToArrowLeftToDetails.delete(contactId)"
            :alive="alive"
          />
        </div>
...
.layout {
  display: grid;
  grid-template-columns: minmax(0, 60dvh) 1fr;
  height: 100dvh;
  width: 100%;
}
