...
export function copyContactCreationDto(dto: ContactCreationDto): ContactCreationDto {
  function sanitizeSpacesUnlessNull(value: Nullable<string>): string | null {
    return value === null || value === undefined ? null : sanitizeSpaces(value)
  }
  ...
  return new ContactCreationDto(
    ...
    sanitizeSpacesUnlessNull(dto.displayName),
    sanitizeSpaces(dto.firstName),
    sanitizeSpaces(dto.lastName),
    sanitizeSpacesUnlessNull(dto.patronymicName),
    dto.timezone
  )
}
